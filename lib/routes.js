...
ROUTES.parse = function parse_route(routes) {
	...
	if(is.obj(routes)) {
		...
		// If it has `.toRoutes()`, we expect it to convert as our routes object.
		if(is.func(routes.toRoutes)) {
			return parse_route(routes.toRoutes());
		}
...
ROUTES.load = function(path, opts) {
	...
	if(FS.existsSync(routes_file)) {
		routes = ROUTES.parse( require( routes_file ) );
	}
	...
	FS.readdirSync(path).forEach(function(filename) {
		...
		// Skip ignored files
		if(opts.ignore(filename)) { return; }
		if(!opts.accept(filename)) { return; }

		// Load subdirs
		if(stat.isDirectory()) {
			routes[filename] = ROUTES.load(full_filename);
			return;
		}
		...
		if(stat.isFile()) {
			...
			if(routes[basename] !== undefined) {
				console.error('Warning! Duplicate route overriding from ' + full_filename );
			}
			routes[basename] = ROUTES.parse(require(full_filename));
			return;
		}
...
ROUTES.setup = function(app, routes, target, opts) {
	...
	Object.keys(routes).forEach(function(k) {
		...
		ROUTES.setup(app, new_route, new_target, opts);
	});
};
