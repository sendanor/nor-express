...
ROUTES.load = function(path) {
	...
	FS.readdirSync(path).forEach(function(filename) {
		...
		var stat = FS.statSync(PATH.resolve(path, filename));
		var ext, basename;
		...
		// Load subdirs
		if(stat.isDirectory()) {
			routes[filename] = ROUTES.load(PATH.resolve(path, filename));
			return;
		}
		...
		if(stat.isFile() && (ext === '.js') ) {
			...
			if(routes[basename] !== undefined) {
				console.error('Warning! Duplicate route overriding from ' + PATH.resolve(path, filename) );
			}
			routes[basename] = ROUTES.parse( require(PATH.resolve(path, filename)) );
			return;
		}
